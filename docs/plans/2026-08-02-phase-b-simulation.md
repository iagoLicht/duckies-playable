...
- **Post-Phase-B change (2026-08-03, user-requested): official aim system.** The placeholder 260 px dotted line is replaced by the official example's trajectory preview (`src/sim/trajectory.ts`, `predictShot`): a 6 px-step swept `DUCK_R` circle, max 1440 px, one wall bounce (second leg max 990 px), stopping at the first duck/barrel; the view draws crawling white dots (36 px spacing, ~100 px/s), an **advisory red X** where the path hits nothing, and a white tapered billiards deflection streak off a struck duck. `MIN_PULL` 25 → 40 (official 0.45 world units ≈ 40 px) so short pulls neither aim nor fire; the X is advisory only — release still fires, matching the official game. `Slingshot.aimDir()` is now the single post-assist direction path shared by `end()` and `preview()`. No balance change: the bot's 140–200 px pulls clear the new threshold and the 600-seed stats are byte-identical — `winRate 1, p50 52.4, p90 69.3, finaleRate 0.985, avgBlasts 9.9`.
- **Post-Phase-B change (2026-08-03, user-requested): official match-fuse collisions.** Same-colour contact no longer pops on impact. It calls `flagMatched` on *both* ducks (`matched = true`, `matchFuse = 90` ticks); the ducks keep **full physics** — they drift, bounce and can flag further ducks — while blinking white, and pop when the fuse hits 0. Each pop's blast relights a *fresh* 90-tick fuse on the same-colour ducks it catches, so a chain costs ~1.5 s per generation instead of `CHAIN_DELAY`'s 0.08 s. Constants are the official example's, converted at our 90 px/unit: `POP_SPEED 120 → 126` (their 1.4 u/s), `BLAST_R 140 → 135` (their 1.5 u), new `MATCH_FUSE_TICKS 90` and `MATCH_BLINK_TICKS 9`; `CHAIN_DELAY` is gone along with the whole `popQueue`/`schedulePop` machinery (`World.popDuck(duck)` is now the public "pop this one now" entry point). Verified frame-by-frame against the shipped example driven through `window.__game`: fuse reads 89 on the first observable tick (their tick order decrements it in the same tick it is lit), the first white band starts at fuse 80, bands are exactly 9 ticks, and the chained duck is flagged on the *same tick* as its parent's pop. View side, all from the decomp: white-out by copying each attachment and forcing its colour to opaque white (this rig keeps the duck's hue in attachment colours over neutral art, so the trick ports 1:1), the rig's own `jump` plus a 1.22× 100 ms yoyo punch and a 0.7-scale duck-tinted star on match, and on pop a warm-white `0xFFE9B8` star, an additive duck-tinted bloom 30→70 px over 220 ms, a 40 ms hitstop and a 70 ms / 0.003-intensity shake. **Balance held without touching a gate:** `winRate 1, p50 47.5, p90 60.8, finaleRate 0.995, avgBlasts 8.2` (vs. 52.4 / 69.3 / 0.985 / 9.9 before) — p50 actually *fell*, because ducks that keep moving while fused detonate spread out across the board instead of at one contact point. The playthrough bot now skips matched ducks when picking a shot, mirroring the UI rule that a matched duck loses its ring and refuses a grab.
- **Follow-up fidelity fix (2026-08-03): blast reach is pure centre distance.** `World.blast()` had been padding the radius with the victim's body radius (`BLAST_R + DUCK_R` = 181 px for ducks, `BLAST_R + BARREL_R` = 195 px for barrels). The official's `explodeAt` (decomp `vault-app.pretty.js` ~l.1105) does no such thing — it is `fr(r.pos, A) > s` with `s = BLAST_RADIUS²`, i.e. a bare **centre-to-centre** test against 1.5 u = **135 px** at our 90 px/unit, with no body-radius terms at all. Both checks are now `Math.hypot(…centre delta…) <= SIM.BLAST_R`. The reach loss is large (−25 % on ducks, −31 % on barrels) but the balance absorbed it with **no gate touched**: `winRate 1, p50 49.4, p90 63.2, finaleRate 0.997, avgBlasts 8.4` (vs. `1 / 47.5 / 60.8 / 0.995 / 8.2` before) — p50 drifts up ~2 s because a few marginal chain links no longer connect, still well inside the committed `p50 < 55` / `p90 < 80` / `finaleRate > 0.85` / `avgBlasts > 4` gates. Two `chains.test.ts` cases needed their *geometry* re-derived (assertions untouched): matched ducks drift for the full 1.5 s fuse, so victims must be placed against the **pop** point, not the spawn point. The chain test's bystanders moved to `(340, 810)` / `(235, 810)` — 117 px and 128 px from the pop at `(301.1, 700)`, and ≥ 92 px clear of the lane the pair runs down. The hp-3 barrel test was the harder one: a head-on pair always ends up ≥ ~210 px apart (92 px contact gap + ≥ 120 px of fuse drift), and the lens where two 135 px discs overlap at that separation is only ±85 px tall — too thin to clear `DUCK_R + BARREL_R` = 106 px, so *no* barrel position can take both blasts without also taking a direct hit. The fix is to shoot hard enough (`533` instead of `140`) that the struck duck rebounds off the far wall and comes most of the way back inside its fuse, closing the two pop points to `(301.8, 700)` and `(403.4, 700)`; the barrel sits at `(353, 815)`, 115 px below the lane and ~126 px from each pop point, and still takes exactly two blast hits (hp 3 → 1).
- **Deliberate deviation: the hitstop freezes the sim only.** The task brief called for a "full-game freeze". The decomp's `GameScene.update` is `this.frozen || this.sim.update(t, this.time2.scale)` — `time2.freeze` zeroes *only* the sim's timescale, so the official's tweens, particles and spine rigs animate straight through the 40 ms. Freezing our fx tickers too would visibly stutter the pop burst, i.e. be *less* identical, so `tick()` gates `director.step` alone.
- **Deliberate deviation: the match test always uses pre-bounce relative speed.** The official has two paths: the generic target-vs-target branch tests the pair's relative speed *before* the impulse, but `shotVsDuck` (used when one duck is the active shot) tests the shot's speed *after* it — and with equal masses at restitution 0.96 a dead-on hit leaves the shooter at ~0, so a perfectly aimed direct hit never matches there. Our sim has no `activeShotId` concept and uses the pre-bounce relative speed everywhere; that is the forgiving superset and what the brief specified.
